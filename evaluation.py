# -*- coding: utf-8 -*-
"""
Created on Wed Aug 31 10:06:23 2022
...
class Graphs:
    def __init__(self, data, N, tmax, edge_contact_time):
        ...
        edge_total_counts = sparse.COO(coords, np.ones((data.shape[0],)).astype(int), shape=(N, N, data.shape[0]))
...
def MMD_Mean_Average_Degree(Gs, FGs):
    return  MMD(
        Gs.Mean_Average_Degree_Distribution().reshape(-1, 1),
        FGs.Mean_Average_Degree_Distribution().reshape(-1, 1)
    )
...
def get_real_graph_data(data_name, time_interval, edge_contact_time, n_samples=1000):
    ...
    if 'auth' in data_name:
        ...
        real_graphs = np.loadtxt('./{}/{}.txt'.format(name, name))
    if 'edmot' in data_name:
        user_id = 0
        #name = '{}_user_{}'.format(data_name, user_id)
        #file = "data/{}_user_{}.txt".format(data_name, user_id)
        real_graphs = np.loadtxt('./data/{}_data_total.txt'.format('edmot'))
    if 'finvolution' in data_name:
        user_id = 0
        #name = '{}_user_{}'.format(data_name, user_id)
        #file = "data/{}_user_{}.txt".format(data_name, user_id)
        real_graphs = np.loadtxt('./data/{}.txt'.format('finvolution'))

    return real_graphs


def get_fake_graph_data(dataset, iter, n_samples=100):
    if 'tggan' in dataset:
        if 'auth' in dataset:
            N = 27
            l = 103
            ...
            thres = 0.001
            _it = '20220807-193406_assembled_graph_iter_{}'.format(iter)
            matlab_file = 'fake_graph_TGGAN_auth_iter_30000'
            dir = 'outputs-auth-user-0'
            ...
            time_interval = 1. / n_times
            thres = 0.001
            _it = '20220622-131711_assembled_graph_iter_{}'.format(iter)
            matlab_file = 'fake_graph_TGGAN_auth_iter_{}'.format(iter)
            dir = 'outputs-edmot_data_total-0'
            fake_file = './{}/{}.npz'.format(dir, _it)
        elif 'finvolution' in dataset:
            N = 128
            l = 103
            n_times = 4
            tmax = 1.0
            ...
            time_interval = 1. / n_times
            thres = 0.001
            _it = '20220617-101100_assembled_graph_iter_{}'.format(iter)
            matlab_file = 'fake_graph_TGGAN_auth_iter_{}'.format(iter)
            dir = 'outputs-finvolution-0'
            fake_file = './{}/{}.npz'.format(dir, _it)
...
if __name__ == "__main__":
    ...
    # train_ratio = 0.8
    #data = pd.DataFrame(columns=['mean_degree','avg_group_size','mean_coord_number','mmd_avg_degree','mean_avg_degree','group_size','mean_group_number','mean_group_duration'])
    List = []
    # º∆À„≤ªÕ¨µ¸¥˙œ¬µƒª˘”⁄MMDµƒ÷∏±Í
    for iteration in range(1000, 9000, 1000):
        tmp = []
        N, n_times, tmax, time_interval, thres, edge_contact_time, fake_graphs = get_fake_graph_data(dataset, iteration)
        ...
        mean_degree = MMD_Mean_Degree(Gs, FGs)
        log("mean_degree: {}".format(mean_degree))
        avg_group_size = MMD_Average_Group_Size(Gs, FGs)
        log("avg_group_size: {}".format(avg_group_size))
        mean_coord_number = MMD_Mean_Coordination_Number(Gs, FGs)
        log("mean_coord_number: {}".format(mean_coord_number))

        mmd_avg_degree = MMD_Average_Degree_Distribution(Gs, FGs)
        log("mmd_avg_degree: {}".format(mmd_avg_degree))
        mean_avg_degree = MMD_Mean_Average_Degree(Gs, FGs)
        log("mean_avg_degree: {}".format(mean_avg_degree))
        group_size = MMD_Group_Size_Distribution(Gs, FGs)
        log("group_size: {}".format(group_size))
        mean_group_number = MMD_Mean_Group_Number(Gs, FGs)
        log("mean_group_number: {}".format(mean_group_number))
        mean_group_duration = MMD_Mean_Group_Duration(Gs, FGs)
        log("mean_group_duration: {}".format(mean_group_duration))
        
        tmp.append(mean_degree)
        tmp.append(avg_group_size)
        tmp.append(mean_coord_number)
        tmp.append(mmd_avg_degree)
        tmp.append(mean_avg_degree)
        tmp.append(group_size)
        tmp.append(mean_group_number)
        tmp.append(mean_group_duration)
        
        List.append(tmp)
        
    data = pd.DataFrame(List, columns=['mean_degree','avg_group_size','mean_coord_number','mmd_avg_degree','mean_avg_degree','group_size','mean_group_number','mean_group_duration'])
    data.to_csv("tggan_reward_auth_eva_1.csv")
