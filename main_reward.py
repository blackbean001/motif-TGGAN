...
parser.add_argument("-gid", "--gpu_id", default=7, type=int)
parser.add_argument("-ep", "--epochs", default=10000, type=int)
parser.add_argument("-lr", "--learningrate", default=0.0001, type=float,
...
parser.add_argument("--early_stopping", default=0.01, type=float,
...
def process_input(real_data, N):
    #real_edge_inputs_discrete = torch.cuda.LongTensor(real_data[:, 1:, 0:2])
    ...
    #real_end_discretes = torch.cuda.LongTensor(real_data[:, 0, 1])
    ...
    return torch.tensor(real_node_inputs), torch.tensor(real_tau_inputs), torch.tensor(real_x_inputs), \
            torch.tensor(real_ends), torch.tensor(real_t0_res_inputs)
...
def run(config, args):
    ...
    if dataname == "auth":
        ...
        if continue_training != '':
            resume_path = "output/{}/save_models/bestmodel-".format(continue_training)
        ...
        for epoch in range(1, epochs):
            ...
            if epoch > 0 and epoch % eval_every == 0:
                ...
                if mmd_avg_degree > mmd_avg_degree_max:
                    mmd_avg_degree_max = max(mmd_avg_degree_max, mmd_avg_degree)
                    save_model_path = "output/{}/save_models/model-snapshots-".format(model_name)
            ...
            if gen_cost < early_stopping and disc_cost < early_stopping:
