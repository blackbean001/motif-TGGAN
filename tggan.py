...
class RewardNet(nn.Module):
    def __init__(self, N, rw_len, reward_every,
        ...
        self.final_score_dense = nn.Linear(self.R_layers[-1], self.num_motif, bias=False)
        self.final_bias = nn.parameter.Parameter(self.real_score, requires_grad=True) # (1, 17)
        print('final_bias.size(): ', self.final_bias.size())
    ...
    def forward(self, x=None, t0_res=None, node_inputs=None, tau_inputs=None, end=None):
        ...
        outputs = inputs
        last_output = outputs[-1] # (bs, self.D_layers[-1])
        final_score = self.final_score_dense(last_output) + self.final_bias  # (bs, 17)
